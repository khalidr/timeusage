...
object TimeUsage {
  ...
  def dfSchema(columnNames: List[String]): StructType = {
    ...
    val rest = columnNames.tail.map(name ⇒ StructField(name, DoubleType, nullable = false))

    StructType.apply(first :: rest)
  }
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    val primaryNeedsColumns = columnNames.filter(name ⇒ name.startsWith("t01")  || name.startsWith("t03") || name.startsWith("t11")).map(col)
    val workingColumns = columnNames.filter(name ⇒ name.startsWith("t05")  || name.startsWith("t1805")).map(col)
    val leisureColumns = columnNames.filter(name ⇒
      name.startsWith("t02") ||
        name.startsWith("t04") ||
        name.startsWith("t06") ||
        name.startsWith("t07") ||
  ...
  def timeUsageSummary(
    ...
    val workingStatusProjection: Column = when(df.col("telfs") >= 1 && df.col("telfs") < 3, "working").otherwise("not working").as("working")
    ...
    val primaryNeedsProjection: Column = (primaryNeedsColumns.tail.foldLeft(primaryNeedsColumns.head){_ + _ } / 60).as("primaryNeeds")
    val workProjection: Column = (workColumns.tail.foldLeft(primaryNeedsColumns.head){_ + _ } / 60).as("work")
    val otherProjection: Column = (otherColumns.tail.foldLeft(primaryNeedsColumns.head){_ + _ } / 60).as("other")
    df
  ...
  def timeUsageGrouped(summed: DataFrame): DataFrame = {
    val df = summed.groupBy( $"age",$"sex", $"working").avg("primaryNeeds", "work", "other").orderBy($"working", $"sex", $"age")
    df.withColumnRenamed("avg(primaryNeeds)", "primaryNeeds")
